...
export default function Header() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 transition-all py-4">
      <div className="max-w-[1920px] mx-auto px-4">
        <div className="relative flex items-center justify-between transition-all duration-300 bg-white/95 shadow-soft rounded-2xl px-4 md:px-8 py-4 backdrop-blur-sm">
          ...
          {isMobileMenuOpen && (
            <div
              ref={mobileMenuRef}
              className="lg:hidden absolute top-full right-0 left-0 md:left-auto mt-2 md:w-64 bg-white rounded-lg shadow-lg overflow-hidden transform origin-top-right transition-all duration-200 mx-0 md:mx-2"
            >
              <nav className="py-2">
                {[...navItems, ["Login", ""]].map(([title, sectionId]) => (
                  <a
                    ...
                    className={`flex items-center px-6 py-4 text-base font-semibold transition-colors duration-200
                      ${title === "Login" ? "border-t border-brand-50" : ""}
                      ${
                        isActive(sectionId)
                          ? "text-brand-500 bg-brand-50"
                          : "text-brand-800 hover:text-brand-500 hover:bg-brand-50"
                      }`}
                  >
                    {title === "Login" && <LogIn className="h-5 w-5 mr-3" />}
                    {title}
                  </a>
                ))}
              </nav>
